...
intentionally skipped and stays skipped). This work ships as
**`0007_public_access_hardening.sql`**, not the old 0005 slot: it alters the
`trainer_documents_insert` policy created in `0006`, so it must run *after* 0006,
and 0005 sorts before it.
