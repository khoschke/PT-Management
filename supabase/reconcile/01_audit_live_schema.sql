...
with checks(migration, item, present) as (
  values
    ...
    ('0004', 'old column trainers.availability dropped',
    ...
    ('0006', 'enum document_expiry_rule',
      to_regtype('public.document_expiry_rule') is not null),
    ('0006', 'enum document_status',
      to_regtype('public.document_status') is not null),
    ('0006', 'table document_types',
      to_regclass('public.document_types') is not null),
    ('0006', 'table trainer_documents',
      to_regclass('public.trainer_documents') is not null),
    ('0006', 'table document_reminders',
      to_regclass('public.document_reminders') is not null),
    ('0006', 'storage bucket trainer-documents',
      exists (select 1 from storage.buckets where id = 'trainer-documents')),
    ('0006', 'storage policy "trainer documents select"',
      exists (select 1 from pg_policies
              where schemaname = 'storage' and tablename = 'objects'
                and policyname = 'trainer documents select')),
    ('0006', 'storage policy "trainer documents insert"',
      exists (select 1 from pg_policies
              where schemaname = 'storage' and tablename = 'objects'
                and policyname = 'trainer documents insert')),
    ('0006', 'storage policy "trainer documents delete"',
      exists (select 1 from pg_policies
              where schemaname = 'storage' and tablename = 'objects'
                and policyname = 'trainer documents delete')),

    -- 0007 / 0008 GymMaster (unmerged branch — expected MISSING) -----------
    ('0007', 'lead_source value gymmaster_api',
      exists (select 1 from pg_enum e
              join pg_type t on t.oid = e.enumtypid
              where t.typname = 'lead_source' and e.enumlabel = 'gymmaster_api')),
    ('0008', 'column leads.gymmaster_member_id',
      exists (select 1 from information_schema.columns
    ...
    ('0009-A', 'function submit_form_lead(...)',
    ...
    ('0009-B', 'policy leads_insert_public_form removed',
      not exists (select 1 from pg_policies
                  where schemaname = 'public' and tablename = 'leads'
                    and policyname = 'leads_insert_public_form')),
    ('0009-B', 'anon cannot insert into leads',
